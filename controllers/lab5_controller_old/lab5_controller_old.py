...
if mode == "planner":
    ...
    end_w = (-1.4, -9.7) # (Pose_X, Pose_Y) in meters
    ...
    # Create the convolved map for configuration space
    ...
    plt.imshow(np.transpose(map), origin="upper")
    # plt.plot(start[0], start[1], "ro", markersize=10)
    
    # Plot the convolved map
    plt.subplot(1, 3, 2)
    plt.title("Configuration Space")
    plt.imshow(convolved_map, origin="upper")
    # plt.plot(start[0], start[1], "ro", markersize=10)
    
    # The third plot will be filled after path planning
    plt.subplot(1, 3, 3)
    ...
    if end_w is None:
        ...
        end = (360 - abs(int(end_w[0] * 30)), abs(int(end_w[1] * 30)))
